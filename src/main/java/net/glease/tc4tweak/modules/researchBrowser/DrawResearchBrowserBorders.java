...
public class DrawResearchBrowserBorders {
    ...
    // each int[] denotes two vector. the dot product with (coord, border size) produce the right coord for a side on
    // the 3x3 grid
        ...
        for (int[] paramX : PARAMS) {
            for (int[] paramY : PARAMS) {
                ClientUtils.drawRectTextured(
                        x + paramX[0] * width + paramX[1] * BORDER_WIDTH,
                        x + paramX[2] * width + paramX[3] * BORDER_WIDTH,
                        y + paramY[0] * height + paramY[1] * BORDER_HEIGHT,
                        y + paramY[2] * height + paramY[3] * BORDER_HEIGHT,
                        paramX[0] * TEXTURE_WIDTH + paramX[1] * BORDER_WIDTH,
                        paramX[2] * TEXTURE_WIDTH + paramX[3] * BORDER_WIDTH,
                        paramY[0] * TEXTURE_HEIGHT + paramY[1] * BORDER_HEIGHT,
                        paramY[2] * TEXTURE_HEIGHT + paramY[3] * BORDER_HEIGHT,
                        BACKGROUND_ZLEVEL);
            }
        }
    ...
    public static void drawResearchBrowserBackground(
        ...
        double x1 = x / scale,
                ...
                u1 = u / scale,
                v1 = v / scale,
                w1 = ConfigurationHandler.INSTANCE.getBrowserWidth() / scale / 2 - BORDER_WIDTH / scale,
                h1 = ConfigurationHandler.INSTANCE.getBrowserHeight() / scale / 2 - BORDER_HEIGHT / scale;
