...
class GuiResearchBrowserVisitor extends ClassVisitor {
    ...
    public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if ("genResearchBackground".equals(name) && "(IIF)V".equals(desc)) {
            ...
            return new GenResearchBackgroundVisitor(
                    api,
                    new ConstantToDynamicReplacer(
                            api, new LimitResearchCategoryToPageVisitor(api, mv, 1), 9, "getTabPerSide", 1));
            ...
            return new ConstantToDynamicReplacer(
                    api,
                    new ConstantToDynamicReplacer(
                            api, new LimitResearchCategoryToPageVisitor(api, mv, 1), 9, "getTabPerSide"),
                    280,
                    "getTabIconDistance");
        }
        return mv;
    ...
    private static class UpdateResearchVisitor extends MethodVisitor {
        ...
        public void visitFieldInsn(int opcode, String owner, String name, String desc) {
            if (opcode == PUTSTATIC
                    && TARGET_INTERNAL_NAME.equals(owner)
                    && name.startsWith("guiMap")
                    && "I".equals(desc)) {
    ...
    private static class GenResearchBackgroundVisitor extends MethodVisitor {
        ...
        public void visitFieldInsn(int opcode, String owner, String name, String desc) {
            super.visitFieldInsn(opcode, owner, name, desc);
            if (opcode == GETFIELD
                    && "thaumcraft/client/gui/GuiResearchBrowser".equals(owner)
                    && "paneWidth".equals(name)
                    && "I".equals(desc)) {
                counterPaneWidth++;
                if (counterPaneWidth > 2)
                    TC4Transformer.log.warn(
                            "GuiResearchBrowser has been changed by other people! Things are not going to work right!");
            }
        }
        ...
        public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
            if (found112
                    && !backgroundDrawChanged
                    && opcode == INVOKEVIRTUAL
                    && owner.equals(TARGET_INTERNAL_NAME)
                    && isDrawTexturedModalRect(name)
                    && desc.equals("(IIIIII)V")) {
                backgroundDrawChanged = true;
                TC4Transformer.log.debug("Deflecting drawTexturedModalRect to drawResearchBrowserBackground");
                super.visitMethodInsn(
                        INVOKESTATIC,
                        ASMCALLHOOK_INTERNAL_NAME,
                        "drawResearchBrowserBackground",
                        "(L" + TARGET_INTERNAL_NAME + ";IIIIII)V",
                        false);
            } else if (!borderDrawChanged
                    && counterPaneWidth == 2
                    && opcode == INVOKEVIRTUAL
                    && owner.equals(TARGET_INTERNAL_NAME)
                    && isDrawTexturedModalRect(name)
                ...
                super.visitMethodInsn(
                    ...
                    && owner.equals("thaumcraft/client/lib/UtilsFX")
                    && name.equals("drawTexturedQuad")
                    && desc.equals("(IIIIIID)V")
                    && counterParticleDraw++ == 1) {
                ...
                super.visitMethodInsn(
                        ...
                        "(IIIIIIDL" + TARGET_INTERNAL_NAME + ";)V",
                        false);
            } else {
    ...
    private static class FieldAccessDeflector extends MethodVisitor {
        private static final Map<String, String> TARGET_FIELDS =
                ImmutableMap.of("paneWidth", "getResearchBrowserWidth", "paneHeight", "getResearchBrowserHeight");
        ...
        public void visitFieldInsn(int opcode, String owner, String name, String desc) {
            ...
            if (opcode == GETFIELD
                    && TARGET_INTERNAL_NAME.equals(owner)
                    && (target = TARGET_FIELDS.get(name)) != null
                    && "I".equals(desc)) {
        ...
        public void visitIntInsn(int opcode, int operand) {
            if (operand == 224 && widthReplaced < 2) {
                ...
                TC4Transformer.log.debug(
                        "Replacing inner height 196 with getResearchBrowserHeight() - 2 * BORDER_HEIGHT");
                super.visitMethodInsn(
                        INVOKESTATIC, ASMCALLHOOK_INTERNAL_NAME, "getResearchBrowserHeight", "()I", false);
    ...
    private static class LimitResearchCategoryToPageVisitor extends MethodVisitor {
        ...
        public void visitFieldInsn(int opcode, String owner, String name, String desc) {
            if (replaced < maxReplacement
                    && opcode == GETSTATIC
                    && "thaumcraft/api/research/ResearchCategories".equals(owner)
                    && "researchCategories".equals(name)
                    && "Ljava/util/LinkedHashMap;".equals(desc)) {
